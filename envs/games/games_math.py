...
class Vector2:
    ...
    def is_normalized(self, tol: float = EPSILON) -> bool:
        """Quickly check if the vector is unit-length (within tolerance)."""
        length_sq = self.x * self.x + self.y * self.y # squared magnitude (avoids sqrt for speed)
    ...
    def cross(self, other: "Vector2") -> float:
        """Return the 2D cross product (scalar), positive if 'other' is to the left."""
    ...
    def angle(self, other: "Vector2", degrees: bool = False) -> float:
        ...
        cosine_of_angle = max(-1.0, min(1.0, cosine_of_angle))  # Clamp for precision safety
        angle_radians = math.acos(cosine_of_angle)
        return math.degrees(angle_radians) if degrees else angle_radians

    def angle_with_direction(self, other: "Vector2", degrees: bool = False) -> float:
    ...
    def rotate(self, angle_radians: float, degrees: bool = False, pivot: "Vector2" = None) -> "Vector2":
